...
/** Único seletor global tolerado, e só no arquivo abaixo (§ 9.4 do contrato). */
...
/**
 * Varre o CSS e devolve cada seletor com a linha onde a regra abre. Ignora o corpo das
 * declarações, de @font-face e de @keyframes (lá "from"/"50%" não são seletores).
 * @param {string} css
 * @param {number} deslocamento linha da primeira linha do css no arquivo original
 * @return {Array<{seletor: string, linha: number}>}
 */
    ...
    if (caractere === "{") {
      ...
      if (ignorando > 0) {
        ...
        for (const parte of texto.split(",")) {
          if (parte.trim() !== "") {
            encontrados.push({seletor: parte.trim(), linha});
          }
        }
        ignorando = 1;
      }
      ...
      // At-rule de sentenca (@import, @charset) termina aqui. Sem este reset, o
      // @import gruda no primeiro seletor do arquivo e ele passa sem ser checado.
      prelude = "";
    } else {
      prelude += caractere;
    }
...
function checarSeletores(css, deslocamento, arquivo, aceitaExcecao) {
  const falhas = [];
  for (const {seletor, linha} of seletoresDeCss(css, deslocamento)) {
    if (seletor.startsWith(".lpb-")) {
      continue;
    }
    if (aceitaExcecao && seletor === SELETOR_EXCECAO) {
      continue;
    }
