...
- [ ] Soft-404: caminho desconhecido (`/guias/nao-existe`) devolve 200 com a landing pelo rewrite da SPA (pré-existente). Opção: prerender `404.html` + restringir o rewrite às rotas da SPA (`/leads`, `/manual-doxa`, `/admin`, `/conversor`) e deixar o resto cair no 404 — cuidado com o `#forms`/hash. (VALIDAR-LIVE)
